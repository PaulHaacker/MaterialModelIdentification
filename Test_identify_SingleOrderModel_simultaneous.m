...
strain_data = G1StressDriven_SingleOrderModel(par2par_norm(par_test),stress_data,time,strain_0);
...
[par_norm_lsqnonlin,res] = identify_SingleOrderModel_simultaneous(creep, DMA, par_norm0);

% verify solution:
disp(['True (normalized) parameters:'])
disp(num2str(par2par_norm(par_test)'))
disp(['Identified (normalized) parameters:'])
disp(num2str(par_norm_lsqnonlin))

disp(['norm of difference = ',num2str(norm(par_norm_lsqnonlin-par2par_norm(par_test)))])
