...
for ii = 1:length(weight_loss_values)
    ...
    % Plot storage modulus
    plot(omega_data, storage_model, '-', 'DisplayName', sprintf('Model, Weight Loss $ W_L = %d$, Parameters: $(\\alpha_1,\\alpha_2,E_0,E_1,p_1,E_2,p_2) = (%s)$', weight_loss, array2strCommas(par_lsqnonlin)));
end
...
for ii = 1:length(weight_loss_values)
    ...
    % Plot loss modulus
    plot(omega_data, loss_model, '-', 'DisplayName', sprintf('Model, Weight Loss $ W_L = %d$, Parameters: $(\\alpha_1,\\alpha_2,E_0,E_1,p_1,E_2,p_2) = (%s)$', weight_loss, array2strCommas(par_lsqnonlin)));
end
...
% 
% par_norm_lsqnonlin = zeros(length(par_0),length(weight_loss_values));
% 
% for ii = 1:length(weight_loss_values)
%     weight_loss = weight_loss_values(ii);
% 
%     par_norm_lsqnonlin(:,ii) = identify_DoubleOrderModel_DMA(omega_data, storage_data, loss_data, par_0, weight_loss);
% end
% 
...
subplot(2, 1, 2);
hold on;
plot(omega_data, loss_model_frac, '-', 'DisplayName', sprintf('fractional-order Model, Weight Loss $ W_L = %d$, Parameters: $(\\alpha_1,\\alpha_2,E_0,E_1,p_1,E_2,p_2) = (%s)$', weight_loss, array2strCommas(par_lsqnonlin_frac)));
